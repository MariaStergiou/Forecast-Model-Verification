...
DIR_DATA  <- here("data")
DIR_DOCS  <- here("output", "WS", "docs")
DIR_PLOTS <- here("output", "WS", "plots")
...
for (mo in MONTHS) {
  ...
  # OBS 
  obs_file <- paste0('WS_OBS_2025_', mo, '.txt')
  obs_table_data <- read.table(
    file = file.path(DIR_DATA, obs_file), 
    skip = 8, sep = "\t", fill = TRUE, quote = ""
  )
  obs_cor_full <- suppressWarnings(data.frame(
    obs_codes = trimws(obs_table_data[, 2]),
    obs_dates = as.Date(trimws(obs_table_data[, 3])),
    obs_ws_table_h = as.numeric(obs_table_data[, 5]),
    obs_ws_table = as.numeric(obs_table_data[, 7]),
    obs_station_names = trimws(obs_table_data[, 10]) # Kept only to be included in the .txt outputs
  ))
  
  # ECMWF
  ecmwf_file <- paste0('WS_ECMWF_2025_', mo, '.txt')
  ecmwf_table_data <- read.table(
    file = file.path(DIR_DATA, ecmwf_file), 
    skip = 9, sep = "\t", fill = TRUE, quote = ""
  )
  ecmwf_cor_full <- suppressWarnings(data.frame(
    ecmwf_codes = trimws(ecmwf_table_data[, 3]),
    ecmwf_dates = as.Date(trimws(ecmwf_table_data[, 5])),
    ecmwf_ws_table_h = as.numeric(ecmwf_table_data[, 6]),
    ecmwf_ws_table = as.numeric(ecmwf_table_data[, 8]),
    ecmwf_station_names = trimws(ecmwf_table_data[, 11])
  ))
  
  # ICON
  icon_file <- paste0('WS_ICON_2025_', mo, '.txt')
  icon_table_data <- read.table(
    file = file.path(DIR_DATA, icon_file), 
    skip = 9, sep = "\t", fill = TRUE, quote = ""
  )
  icon_cor_full <- suppressWarnings(data.frame(
    icon_codes = trimws(icon_table_data[, 3]),
    icon_dates = as.Date(trimws(icon_table_data[, 5])),
    icon_ws_table_h = as.numeric(icon_table_data[, 6]),
    icon_ws_table = as.numeric(icon_table_data[, 8]),
  ...
  for (i in TARGET_HOURS) {
    ...
    # Filter observations
    obs_cor <- obs_cor_full[obs_cor_full$obs_ws_table_h == obs_h, ]
    
    # ---------- Calculations for ECMWF ----------
    ecmwf_cor <- ecmwf_cor_full[ecmwf_cor_full$ecmwf_ws_table_h == i, ]
    ecmwf_cor$valid_date <- ecmwf_cor$ecmwf_dates + days_ahead
    
    # MERGE USING ONLY CODE AND DATE
    final_ecmwf <- merge(x = obs_cor, y = ecmwf_cor, 
                         by.x = c("obs_codes", "obs_dates"), 
                         by.y = c("ecmwf_codes", "valid_date"))
    
    final_ecmwf[final_ecmwf == -999 | final_ecmwf == -99.9] <- NA
    final_ecmwf <- na.omit(final_ecmwf)
    
    if (nrow(final_ecmwf) > 0) {
      rmse_ecmwf <- sqrt(mean((final_ecmwf$ecmwf_ws_table - final_ecmwf$obs_ws_table)^2, na.rm = TRUE))
      me_ecmwf <- mean(final_ecmwf$ecmwf_ws_table - final_ecmwf$obs_ws_table, na.rm = TRUE)
      ...
      doc_filename_ecmwf <- paste0("comparison_result_ws_", mo, "_ecmwf_", i, ".txt")
      write.table(final_ecmwf, file = file.path(DIR_DOCS, doc_filename_ecmwf), 
                  sep = "\t", row.names = FALSE, quote = FALSE)
    }
    
    # ---------- Calculations for ICON ----------
    icon_cor <- icon_cor_full[icon_cor_full$icon_ws_table_h == i, ]
    icon_cor$valid_date <- icon_cor$icon_dates + days_ahead
    
    # MERGE USING ONLY CODE AND DATE
    final_icon <- merge(x = obs_cor, y = icon_cor, 
                        by.x = c("obs_codes", "obs_dates"), 
                        by.y = c("icon_codes", "valid_date"))
    
    final_icon[final_icon == -999 | final_icon == -99.9] <- NA
    final_icon <- na.omit(final_icon)
    
    if (nrow(final_icon) > 0) {
      rmse_icon <- sqrt(mean((final_icon$icon_ws_table - final_icon$obs_ws_table)^2, na.rm = TRUE))
      me_icon <- mean(final_icon$icon_ws_table - final_icon$obs_ws_table, na.rm = TRUE)
      metrics_icon <- rbind(metrics_icon, data.frame(Hour = i, RMSE = rmse_icon, ME = me_icon))
      
      # Save file with the month indicator into the docs directory
      doc_filename_icon <- paste0("comparison_result_ws_", mo, "_icon_", i, ".txt")
  ...
  plot_filename <- paste0("plot_icon_ecmwf_ws_", mo, "_0_48.png")
  ...
  plot_title_rmse <- paste0("Wind Speed (WS) RMSE Evolution (0h - 48h) \n(IFS ECMWF - ICON-GR, ", MONTH_LABELS[mo], ")")
  
  plot(metrics_ecmwf$Hour, metrics_ecmwf$RMSE, type = "b", col = "red", pch = 16,
       xlab = "Forecast Hour (Lead Time)", ylab = "RMSE (m/s)", 
       main = plot_title_rmse,
       xaxt = "n", 
       xlim = c(0, 48), # Fixed axis 0-48
       ylim = c(ylim_rmse[1]*0.9, ylim_rmse[2]*1.15)) 
  ...
  plot_title_me <- paste0("Wind Speed (WS) Mean Error Evolution (0h - 48h) \n(IFS ECMWF - ICON-GR, ", MONTH_LABELS[mo], ")")
  
  plot(metrics_ecmwf$Hour, metrics_ecmwf$ME, type = "b", col = "red", pch = 16,
       xlab = "Forecast Hour (Lead Time)", ylab = "Mean Error (m/s)", 
       main = plot_title_me,
       xaxt = "n", 
       xlim = c(0, 48), # Fixed axis 0-48
       ylim = c(ylim_me[1]-0.5, ylim_me[2]+0.7)) # Give +0.7 extra margin on top so the line doesn't stick
