...
for (mo in MONTHS) {
  ...
  for (i in TARGET_HOURS) {
    ...
    if (nrow(final_ecmwf) > 0) {
      ...
      doc_filename_ecmwf <- paste0("apotelesma_sygkrisis_ws_", mo, "_ecmwf_", i, ".txt")
    ...
    if (nrow(final_icon) > 0) {
      ...
      # Save file with the month indicator into the docs directory
      doc_filename_icon <- paste0("apotelesma_sygkrisis_ws_", mo, "_icon_", i, ".txt")
  ...
  plot_title_rmse <- paste0("Εξέλιξη RMSE Έντασης Ανέμου (0h - 48h) \n(IFS ECMWF - ICON-GR, ", MONTH_LABELS[mo], ")")
  
  plot(metrics_ecmwf$Hour, metrics_ecmwf$RMSE, type = "b", col = "red", pch = 16,
       xlab = "Ώρα Πρόγνωσης (Lead Time)", ylab = "RMSE (m/s)", 
  ...
  plot_title_me <- paste0("Εξέλιξη Mean Error Έντασης Ανέμου (0h - 48h) \n(IFS ECMWF - ICON-GR, ", MONTH_LABELS[mo], ")")
  
  plot(metrics_ecmwf$Hour, metrics_ecmwf$ME, type = "b", col = "red", pch = 16,
       xlab = "Ώρα Πρόγνωσης (Lead Time)", ylab = "Mean Error (m/s)", 
       main = plot_title_me,
       xaxt = "n", 
       xlim = c(0, 48), # Fixed axis 0-48
