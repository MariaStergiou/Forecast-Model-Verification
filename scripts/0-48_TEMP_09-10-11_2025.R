...
DIR_DATA  <- here("data")
DIR_DOCS  <- here("output", "TEMP", "docs")
DIR_PLOTS <- here("output", "TEMP", "plots")
...
for (mo in MONTHS) {
  ...
  # OBS
  obs_file <- paste0('TEMP_OBS_2025_', mo, '.txt')
  ...
  obs_cor_full <- suppressWarnings(data.frame(
    ...
    obs_temp_table_h = as.numeric(obs_table_data[, 5]),
    obs_temp_table = as.numeric(obs_table_data[, 7]),
    obs_station_names = trimws(obs_table_data[, 10]) # Kept only to be included in the .txt outputs
  ))
  
  # ECMWF
  ecmwf_file <- paste0('TEMP_ECMWF_2025_', mo, '.txt')
  ...
  ecmwf_cor_full <- suppressWarnings(data.frame(
    ...
    ecmwf_temp_table_h = as.numeric(ecmwf_table_data[, 6]),
    ecmwf_temp_table = as.numeric(ecmwf_table_data[, 8]),
    ecmwf_station_names = trimws(ecmwf_table_data[, 11])
  ))
  
  # ICON
  icon_file <- paste0('TEMP_ICON_2025_', mo, '.txt')
  ...
  icon_cor_full <- suppressWarnings(data.frame(
    ...
    icon_temp_table_h = as.numeric(icon_table_data[, 6]),
    icon_temp_table = as.numeric(icon_table_data[, 8]),
  ...
  for (i in TARGET_HOURS) {
    ...
    # Filter observations
    obs_cor <- obs_cor_full[obs_cor_full$obs_temp_table_h == obs_h, ]
    
    # ---------- Calculations for ECMWF ----------
    ecmwf_cor <- ecmwf_cor_full[ecmwf_cor_full$ecmwf_temp_table_h == i, ]
    ...
    if (nrow(final_ecmwf) > 0) {
      rmse_ecmwf <- sqrt(mean((final_ecmwf$ecmwf_temp_table - final_ecmwf$obs_temp_table)^2, na.rm = TRUE))
      me_ecmwf <- mean(final_ecmwf$ecmwf_temp_table - final_ecmwf$obs_temp_table, na.rm = TRUE)
      ...
      doc_filename_ecmwf <- paste0("comparison_result_temp_", mo, "_ecmwf_", i, ".txt")
      write.table(final_ecmwf, file = file.path(DIR_DOCS, doc_filename_ecmwf), 
                  sep = "\t", row.names = FALSE, quote = FALSE)
    }
    
    # ---------- Calculations for ICON ----------
    icon_cor <- icon_cor_full[icon_cor_full$icon_temp_table_h == i, ]
    ...
    if (nrow(final_icon) > 0) {
      rmse_icon <- sqrt(mean((final_icon$icon_temp_table - final_icon$obs_temp_table)^2, na.rm = TRUE))
      me_icon <- mean(final_icon$icon_temp_table - final_icon$obs_temp_table, na.rm = TRUE)
      metrics_icon <- rbind(metrics_icon, data.frame(Hour = i, RMSE = rmse_icon, ME = me_icon))
      
      # Save file with the month indicator into the docs directory
      doc_filename_icon <- paste0("comparison_result_temp_", mo, "_icon_", i, ".txt")
  ...
  plot_filename <- paste0("plot_icon_ecmwf_temp_", mo, "_0_48.png")
  ...
  # --- TOP ROW: RMSE Plot ---
  plot_title_rmse <- paste0("Temperature (TEMP) RMSE Evolution (0h - 48h) \n(IFS ECMWF - ICON-GR, ", MONTH_LABELS[mo], ")")
  ...
  plot_title_me <- paste0("Temperature (TEMP) Mean Error Evolution (0h - 48h) \n(IFS ECMWF - ICON-GR, ", MONTH_LABELS[mo], ")")
  ...
  plot(metrics_ecmwf$Hour, metrics_ecmwf$ME, type = "b", col = "red", pch = 16,
       ...
       xaxt = "n", 
       xlim = c(0, 48), # Fixed axis 0-48
       ylim = c(ylim_me[1]-0.5, ylim_me[2]+0.5))
