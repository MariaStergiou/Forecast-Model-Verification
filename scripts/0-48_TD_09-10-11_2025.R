...
for (mo in MONTHS) {
  ...
  # OBS
  obs_file <- paste0('TD_OBS_2025_', mo, '.txt')
  ...
  obs_cor_full <- data.frame(
    ...
    obs_td_table_h = as.numeric(obs_table_data[, 5]),
    obs_td_table = as.numeric(obs_table_data[, 7]),
    obs_station_names = trimws(obs_table_data[, 10])
  )
  
  # ECMWF
  ecmwf_file <- paste0('TD_ECMWF_2025_', mo, '.txt')
  ...
  ecmwf_cor_full <- data.frame(
    ...
    ecmwf_td_table_h = as.numeric(ecmwf_table_data[, 6]),
    ecmwf_td_table = as.numeric(ecmwf_table_data[, 8]),
    ecmwf_station_names = trimws(ecmwf_table_data[, 11])
  )
  
  # ICON
  icon_file <- paste0('TD_ICON_2025_', mo, '.txt')
  ...
  icon_cor_full <- data.frame(
    ...
    icon_td_table_h = as.numeric(icon_table_data[, 6]),
    icon_td_table = as.numeric(icon_table_data[, 8]),
  ...
  for (i in TARGET_HOURS) {
    ...
    # Filter observations
    obs_cor <- obs_cor_full[obs_cor_full$obs_td_table_h == obs_h, ]
    
    # ---------- Calculations for ECMWF ----------
    ecmwf_cor <- ecmwf_cor_full[ecmwf_cor_full$ecmwf_td_table_h == i, ]
    ...
    if (nrow(final_ecmwf) > 0) {
      rmse_ecmwf <- sqrt(mean((final_ecmwf$ecmwf_td_table - final_ecmwf$obs_td_table)^2, na.rm = TRUE))
      me_ecmwf <- mean(final_ecmwf$ecmwf_td_table - final_ecmwf$obs_td_table, na.rm = TRUE)
    ...
    icon_cor <- icon_cor_full[icon_cor_full$icon_td_table_h == i, ]
    ...
    if (nrow(final_icon) > 0) {
      rmse_icon <- sqrt(mean((final_icon$icon_td_table - final_icon$obs_td_table)^2, na.rm = TRUE))
      me_icon <- mean(final_icon$icon_td_table - final_icon$obs_td_table, na.rm = TRUE)
  ...
  plot_filename <- paste0("plot_icon_ecmwf_td_", mo, "_0_48.png")
